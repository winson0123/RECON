...
export async function POST(req: Request) {
  try {
    ...
    const dbFileType = await fileTypeDetector.fromBuffer(dbBuffer) || { ext: "" }
    const ssFileType = await fileTypeDetector.fromBuffer(ssBuffer) || { ext: "" }
    if (dbFileType?.ext !== 'sqlite') {
      return NextResponse.json({ status: "fail", error: "The DB file must be an SQLite (.db) file" });
    } else if (ssFileType?.ext !== 'zip') {
      return NextResponse.json({ status: "fail", error: "The Screenshot file must be a zip (.zip) file" });
    }

    await fs.mkdir(`./tmp/uploads/${index}/screenshots`, { recursive: true }).catch(console.error)
    await fs.writeFile(`./tmp/uploads/${index}/${dbFile.name}`, dbBuffer);
    await jszip.loadAsync(ssArrayBuffer).then((zip) => {
      Object.keys(zip.files).forEach((filename) => {
        zip.files[filename].async('string').then((fileData) => {
          fs.writeFile(`./tmp/uploads/${index}/screenshots/${filename}`, fileData);
        })
      }) 
    })
